...
interface AnalyticsData {
  kpi: {
    ...
    complied: number;
    awaiting: number;
    chargeMemoDue: number;
    drafts: number;
    rejected: number;
  };
  statusCounts: Record<string, number>;
  zoneBreakdown: { zone: string; memos: number; complied: number; awaiting: number }[];
  roleBreakdown: { role: string; count: number }[];
  socialViceBreakdown: { category: string; memos: number }[];
  complianceTrend: { date: string; issued: number; complied: number }[];
  chargeMemoOfficers: {
    officerId: string;
    name: string;
    badgeNumber: string;
...
const ROLE_COLORS: Record<string, string> = { SHO: '#003366', SI: '#d97706', UNASSIGNED: '#94a3b8' };
...
const Dashboard: React.FC = () => {
  ...
  const kpi = data?.kpi || {
    totalMemos: 0, pendingReview: 0, approved: 0, complied: 0,
    awaiting: 0, chargeMemoDue: 0, drafts: 0, rejected: 0,
  };
  const complianceRate = kpi.approved > 0
    ? ((kpi.complied / kpi.approved) * 100).toFixed(1)
    : '0.0';

  const viceData = useMemo(() => {
    const apiList = data?.socialViceBreakdown || [];
    const map = new Map(apiList.map((v) => [v.category, v.memos]));
    const list = VICE_CATEGORIES
      .map((cat) => ({ category: cat, memos: map.get(cat) || 0 }))
      .sort((a, b) => b.memos - a.memos);
    const max = list.reduce((m, x) => Math.max(m, x.memos), 0) || 1;
    return { list, max, topCategory: list[0]?.memos > 0 ? list[0] : undefined };
  }, [data]);
  ...
  const zoneData = useMemo(() => {
    ...
    const list = ALL_ZONES
      .map((z) => ({ zone: z, memos: map.get(z)?.memos || 0, complied: map.get(z)?.complied || 0, awaiting: map.get(z)?.awaiting || 0 }))
      .sort((a, b) => b.memos - a.memos);
    const max = list.reduce((m, x) => Math.max(m, x.memos), 0) || 1;
    return { list, max };
  }, [data]);
  ...
  return (
    <div className="flex flex-col -mx-3 sm:-mx-4 md:-mx-6 -mt-3 sm:-mt-4 md:-mt-6 min-h-full bg-[#f0f2f5]">
      {/* ====== HEADER BAR ====== */}
      <div className="bg-[#003366] px-4 md:px-6 py-3 flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-3">
          <div className="w-1 h-6 bg-[#d4a017] rounded" />
          <h1 className="text-[13px] font-bold text-white tracking-wide uppercase">
            Centralized Social Vice & Memo Analytics Platform
          </h1>
        </div>
        <div className="flex items-center gap-3">
          <span className="bg-white/10 text-white/80 text-[10px] font-medium px-3 py-1 rounded border border-white/20">
            {format(new Date(), 'MMM yyyy')}
          </span>
          <span className="inline-flex items-center gap-1.5 bg-emerald-500/20 text-emerald-300 text-[9px] font-bold uppercase px-2.5 py-1 rounded-full">
      ...
      <div className="px-3 sm:px-4 md:px-6 py-3 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <SectionCard title="Social Vice Category Distribution">
            {viceData.list.length === 0 ? (
              <EmptyState />
            ) : (
              <>
                <div className="space-y-2.5">
                  {viceData.list.map((v, i) => (
                    <HBar
                      key={v.category}
                      label={v.category}
                      value={v.memos}
                ...
                {viceData.topCategory && (
                  <div className="mt-3 pt-2 border-t border-slate-100 flex items-center gap-1.5 text-[10px] text-slate-500">
                    <span className="w-4 h-4 rounded-full bg-blue-50 flex items-center justify-center text-blue-600 text-[9px] font-bold flex-shrink-0">
                      i
                    </span>
                    Top Issue Category:{' '}
                    <span className="font-bold text-slate-800">
                      {viceData.topCategory.category} (
          ...
          <SectionCard title="Zone-wise Issued Memos">
            {zoneData.list.length === 0 ? (
              <EmptyState />
            ) : (
              <div className="space-y-2.5">
                {zoneData.list.map((z, i) => (
                  <HBar
                    key={z.zone}
                    label={z.zone}
                    value={z.memos}
          ...
          <SectionCard title="Role-wise Memo Distribution">
            {roleData.length === 0 ? (
              <EmptyState />
            ) : (
              <>
                <ResponsiveContainer width="100%" height={170}>
                  <BarChart data={roleData} barSize={50}>
                    ...
                    <Bar dataKey="count" name="Memos" radius={[4, 4, 0, 0]}>
                      {roleData.map((r) => (
                ...
                <div className="flex items-center justify-center gap-4 mt-2">
                  {roleData.map((r) => (
                    <div key={r.role} className="flex items-center gap-1.5 text-[9px] text-slate-500">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-2">
          ...
          <SectionCard title="Memo Compliance Status Tracking">
            <div className="flex items-stretch min-h-[220px]">
              ...
              <div className="flex-1 flex flex-col">
                <div className="flex items-center flex-1">
                  ...
                  <div className="flex flex-col items-center min-w-[48px]">
                    <span className="text-[16px] font-extrabold text-amber-600 leading-none">
                      {(100 - parseFloat(complianceRate)).toFixed(0)}%
                    </span>
                    <span className="text-[10px] font-semibold text-amber-500">({kpi.awaiting})</span>
                  </div>
                          ...
                          <Label
                            content={() => (
                              <text x="50%" y="50%" textAnchor="middle" dominantBaseline="middle">
                                <tspan x="50%" dy="-5" fontSize="8" fill="#94a3b8" fontWeight="700">MEMO</tspan>
                                <tspan x="50%" dy="11" fontSize="8" fill="#94a3b8" fontWeight="700">COMPLIANCE</tspan>
                              </text>
                            )}
                            position="center"
                          />
                  ...
                  <div className="flex flex-col items-center min-w-[48px]">
                    <span className="text-[16px] font-extrabold text-emerald-600 leading-none">
                      {parseFloat(complianceRate).toFixed(0)}%
                    </span>
                    <span className="text-[10px] font-semibold text-emerald-500">({kpi.complied})</span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-center gap-6 mt-1">
                  <span className="flex items-center gap-1.5 text-[10px]">
                    ...
                    <span className="text-slate-700 font-bold uppercase">Complied</span>
                  </span>
                  <span className="flex items-center gap-1.5 text-[10px]">
                    <span className="w-3 h-3 rounded-full bg-amber-500" />
                    <span className="text-slate-700 font-bold uppercase">Pending</span>
                  </span>
                </div>
              </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center py-6 px-4">
                ...
                <p className="text-[56px] font-extrabold text-[#003366] tabular-nums leading-none">
                  {isLoading ? '-' : kpi.pendingReview}
                </p>
                <div className="mt-4 flex items-center gap-4">
                  <span className="text-[10px] font-bold text-red-600 bg-red-50 border border-red-200 px-3 py-1 rounded">
                    High Priority
                  </span>
                  <button
                    onClick={() => navigate('/review')}
          ...
          <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-100 gap-2">
              ...
              <span className="bg-[#003366] text-white text-[8px] font-bold px-2.5 py-1 rounded leading-tight text-center uppercase flex-shrink-0">
                    ...
                    <table className="w-full text-[10px]">
                      <thead>
                        <tr className="bg-slate-50">
                          <th className="text-left py-2 px-3 font-bold text-slate-600 uppercase border-b border-slate-200">
                            Officer Name
                          </th>
                          <th className="text-left py-2 px-3 font-bold text-slate-600 uppercase border-b border-slate-200">
                            Designation
                          ...
                          <th className="text-center py-2 px-3 font-bold text-slate-600 uppercase border-b border-slate-200">
                            Pending Memos
                          </th>
                          <th className="text-center py-2 px-3 font-bold text-slate-600 uppercase border-b border-slate-200">
                            Status
                        ...
                        {officers.map((o, idx) => (
                          ...
                          >
                            <td className="py-2 px-3 font-semibold text-slate-800">
                              {rankPrefix(o.rank)} {o.name || '-'}
                            </td>
                            <td className="py-2 px-3 text-slate-600">
                              {designationLabel(o.rank)}
                            </td>
                            <td className="py-2 px-3 text-slate-600">
                              {o.zone || '-'}{o.policeStation ? ` / ${o.policeStation}` : ''}
                            </td>
                            <td className="py-2 px-3 text-center">
                              <span className="inline-flex items-center gap-1.5">
                                <span className="font-bold text-slate-800">{o.memoCount}</span>
                                <span
                                  className={`w-2 h-2 rounded-full ${
                                    o.memoCount >= 5 ? 'bg-red-500' : 'bg-amber-400'
                                  }`}
                                />
                              </span>
                            </td>
                            <td className="py-2 px-3 text-center">
                              <span className={`text-[9px] font-bold px-2 py-0.5 rounded ${
                                ...
                                {o.memoCount >= 5 ? 'Critical' : 'Warning'}
                              </span>
                            </td>
                          </tr>
                        ))}
...
const KPICard: React.FC<{
  ...
  valueColor?: string;
}> = ({ label, value, valueText, sub, isLoading, onClick, icon, valueColor }) => (
  <div
    onClick={onClick}
  ...
  >
    ...
    <div className="px-3 py-2.5">
      ...
      <div className="flex items-baseline gap-2 mt-1">
        <p className={`text-[26px] font-extrabold tabular-nums leading-tight ${valueColor || 'text-[#003366]'}`}>
          {isLoading ? '-' : valueText || (value?.toLocaleString() ?? '0')}
        </p>
...
const HBar: React.FC<{
  ...
  return (
    <div className="flex items-center gap-2 text-[11px]">
      ...
      <span className="text-slate-900 font-bold tabular-nums w-[32px] text-right flex-shrink-0">
        {value}
      </span>
    </div>
  );
...
const EmptyState: React.FC = () => (
  <div className="py-6 text-center">
    <p className="text-[11px] text-slate-400 font-medium">No data available</p>
  </div>
);
...
function designationLabel(rank?: string): string {
  if (!rank) return '-';
  const map: Record<string, string> = {
    INSPECTOR: 'Inspector',
    CI: 'Inspector',
