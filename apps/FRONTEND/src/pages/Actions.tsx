    ...
    <div>
        ...
        {['', 'WARNING', 'SUSPENSION', 'SHOW_CAUSE', 'COUNSELING'].map((t) => (
          <button
            ...
            className={`px-4 py-2 text-xs font-bold uppercase tracking-wider transition ${
              typeFilter === t ? 'bg-indigo-600 text-white' : 'bg-white text-slate-600 border border-slate-200 hover:bg-blue-50/60'
            }`}
          >
            {t === '' ? 'All' : t.replace(/_/g, ' ')}
          </button>
        ...
        <table className="w-full text-sm min-w-[700px]">
          <thead className="bg-gradient-to-r from-[#1a2a4a] to-[#2d4a6f] text-white">
            <tr className="text-left">
              ...
              <th className="px-4 py-3 text-[11px] font-bold uppercase tracking-wider">Type</th>
              ...
              <th className="px-4 py-3 text-[11px] font-bold uppercase tracking-wider">Deadline</th>
              <th className="px-4 py-3 text-[11px] font-bold uppercase tracking-wider">Document</th>
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={9} className="px-4 py-12 text-center text-slate-400">No actions found. Warnings will appear here when incidents are logged.</td></tr>
            ) : (
              actions.map((a, idx) => (
                <tr key={a._id} className={`border-b border-slate-200 hover:bg-blue-50/60 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}`}>
                  <td className="px-4 py-3 text-slate-600">{new Date(a.issuedAt).toLocaleDateString('en-IN')}</td>
                  <td className="px-4 py-3 font-medium">{a.officer?.name || '—'}</td>
                  <td className="px-4 py-3 font-mono text-slate-500">{a.officer?.badgeNumber || '—'}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-block px-2.5 py-1 text-xs font-semibold ${getTypeStyle(a.actionType)}`}>
                  ...
                  <td className="px-4 py-3 text-center font-bold text-slate-700">{a.actionNumber}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-block px-2 py-0.5 border text-xs font-medium ${getStatusStyle(a.status)}`}>
                      {a.status}
                    </span>
                  </td>
      ...
      {pagination.total > 20 && (
        <div className="flex justify-center gap-2 mt-4">
          <button disabled={page <= 1} onClick={() => setPage(page - 1)} className="px-3 py-1 border border-slate-200 disabled:opacity-50 text-[12px] font-bold text-slate-600 uppercase tracking-wider">Prev</button>
          <span className="px-3 py-1 text-[12px] font-bold text-slate-600 uppercase tracking-wider">Page {page}</span>
          <button disabled={page * 20 >= pagination.total} onClick={() => setPage(page + 1)} className="px-3 py-1 border border-slate-200 disabled:opacity-50 text-[12px] font-bold text-slate-600 uppercase tracking-wider">Next</button>
        </div>
      )}
          ...
          <div className="bg-white shadow-xl w-[95vw] max-w-md overflow-hidden">
            ...
            <div className="p-5 space-y-3">
              <div>
                ...
                <select value={editForm.actionType} onChange={(e) => setEditForm({ ...editForm, actionType: e.target.value })} className="w-full border border-slate-300 px-3 py-2 text-sm">
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1">Response Deadline</label>
                <input type="date" value={editForm.responseDeadline} onChange={(e) => setEditForm({ ...editForm, responseDeadline: e.target.value })} className="w-full border border-slate-300 px-3 py-2 text-sm" />
              </div>
            </div>
