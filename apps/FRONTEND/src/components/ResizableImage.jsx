...
function getHandleStyle(h) {
  ...
  if ("top" in h && h.top !== void 0) s.top = h.top;
  if ("bottom" in h && h.bottom !== void 0) s.bottom = h.bottom;
  if ("left" in h && h.left !== void 0) s.left = h.left;
  if ("right" in h && h.right !== void 0) s.right = h.right;
  if ("ml" in h) s.marginLeft = h.ml;
  if ("mt" in h) s.marginTop = h.mt;
  return s;
}
const ResizableImageView = ({ node, updateAttributes, selected, deleteNode }) => {
  ...
  const handleAlign = (a) => updateAttributes({ alignment: a });
  ...
  const wrapperStyle = isInline ? { display: "inline-flex", verticalAlign: "bottom" } : alignment === "center" ? { display: "flex", justifyContent: "center" } : alignment === "right" ? { display: "flex", justifyContent: "flex-end" } : { display: "flex", justifyContent: "flex-start" };
  const btnCls = (active) => `h-6 w-6 flex items-center justify-center rounded transition-colors ${active ? "bg-blue-100 text-blue-700" : "text-slate-600 hover:bg-blue-50"}`;
  ...
  ><div
    ...
    style={{
      width: displayW || void 0,
      height: displayH || void 0,
      maxWidth: "100%"
    }}
  >{
    /* ═══ Floating Image Toolbar ═══ */
  }{selected && !resizing && <div
  ...
  ><div className="flex items-center gap-0.5 bg-white border border-slate-200 rounded-lg shadow-lg px-1.5 py-1 text-[11px]">{
    /* Alignment */
  }<button className={btnCls(alignment === "inline")} title="Inline" onMouseDown={(e) => {
    e.preventDefault();
    handleAlign("inline");
  }}><MoveHorizontal size={13} /></button><button className={btnCls(alignment === "left")} title="Align Left" onMouseDown={(e) => {
    e.preventDefault();
    handleAlign("left");
  }}><AlignLeft size={13} /></button><button className={btnCls(alignment === "center")} title="Center" onMouseDown={(e) => {
    e.preventDefault();
    handleAlign("center");
  }}><AlignCenter size={13} /></button><button className={btnCls(alignment === "right")} title="Align Right" onMouseDown={(e) => {
    e.preventDefault();
    handleAlign("right");
  }}><AlignRight size={13} /></button><div className="w-px h-5 bg-slate-200 mx-0.5" />{
    /* Size presets */
  }<button onMouseDown={(e) => {
    e.preventDefault();
    setSize("25%");
  }} className="h-6 px-1.5 rounded hover:bg-blue-50 text-slate-600 font-medium" title="25% width">25%</button><button onMouseDown={(e) => {
    e.preventDefault();
    setSize("50%");
  }} className="h-6 px-1.5 rounded hover:bg-blue-50 text-slate-600 font-medium" title="50% width">50%</button><button onMouseDown={(e) => {
    e.preventDefault();
    setSize("75%");
  }} className="h-6 px-1.5 rounded hover:bg-blue-50 text-slate-600 font-medium" title="75% width">75%</button><button onMouseDown={(e) => {
    e.preventDefault();
    setSize("100%");
  }} className="h-6 px-1.5 rounded hover:bg-blue-50 text-slate-600 font-medium" title="Full width">100%</button><div className="w-px h-5 bg-slate-200 mx-0.5" />{
    /* Manual W / H */
  }<div className="flex items-center gap-1"><span className="text-slate-400">W</span><input
    type="number"
    value={displayW || ""}
    onChange={(e) => {
  ...
  /><span className="text-slate-400">H</span><input
    type="number"
    value={displayH || ""}
    onChange={(e) => {
      const v = parseInt(e.target.value) || 100;
    ...
    onMouseDown={(e) => e.stopPropagation()}
    onClick={(e) => e.stopPropagation()}
  /></div><div className="w-px h-5 bg-slate-200 mx-0.5" />{
    /* Reset */
  }<button className={btnCls(false)} title="Reset Size" onMouseDown={(e) => {
    e.preventDefault();
    handleResetSize();
  }}><RotateCcw size={13} /></button>{
    /* Delete */
  }<button className="h-6 w-6 flex items-center justify-center rounded hover:bg-red-50 text-red-500" title="Delete" onMouseDown={(e) => {
    e.preventDefault();
    deleteNode();
  }}><Trash2 size={13} /></button></div></div>}<img
    ref={imgRef}
    src={node.attrs.src}
    alt={node.attrs.alt || ""}
...
export const ResizableImage = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    const attrs = { ...rest, style: style.join("; ") || void 0 };
