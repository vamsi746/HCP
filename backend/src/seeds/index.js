...
// ── Seed Function ──────────────────────────────────────────────────────────
async function seed() {
  await _database.connectDB.call(void 0, );
  console.log('Connected to MongoDB. Seeding...');

  // Clear all collections (preserve admin account)
  await Promise.all([
    _models.Officer.deleteMany({ badgeNumber: { $ne: 'admin@hcp.com' } }),
    _models.SectorOfficer.deleteMany({}),
    _models.Sector.deleteMany({}),
    _models.PoliceStation.deleteMany({}),
    _models.Circle.deleteMany({}),
    _models.Division.deleteMany({}),
    _models.Zone.deleteMany({}),
  ]);

  // Drop stale indexes
  try { await _models.Officer.collection.dropIndexes(); } catch (e) { /* ignore */ }
  try { await _models.PoliceStation.collection.dropIndexes(); } catch (e2) { /* ignore */ }

  const passwordHash = await _bcryptjs2.default.hash('Admin@123', 12);
  const commissionerHash = await _bcryptjs2.default.hash('hcp@123', 12);
  let badgeCounter = 1;

  // Create Commissioner (login account)
  const commissioner = await _models.Officer.create({
  ...
  for (const zoneData of ZONES) {
    // Create Zone
    const zone = await _models.Zone.create({ name: zoneData.name, code: zoneData.code });

    // Create one Division + Circle per Zone (simplified hierarchy)
    const division = await _models.Division.create({
      name: `${zoneData.name} Division`,
      code: `${zoneData.code}-D1`,
      zoneId: zone._id,
    });
    const circle = await _models.Circle.create({
    ...
    for (const stationData of zoneData.stations) {
      ...
      const station = await _models.PoliceStation.create({
      ...
      for (const sName of sectorNames) {
        const sector = await _models.Sector.create({ name: sName, policeStationId: station._id });
      ...
      for (const off of stationData.officers) {
        ...
        const officer = await _models.Officer.create({
          name: off.name,
          badgeNumber: badge,
          rank: off.rank || 'SI',
          phone: off.phone,
        ...
        for (const n of sectorNums) {
          ...
          if (sector) {
            await _models.SectorOfficer.create({ sectorId: sector._id, officerId: officer._id, role: 'PRIMARY_SI' });
            assigned = true;
          }
        }
        if (!assigned && Object.values(sectorMap)[0]) {
          // Admin, DSI, Maternity Leave, General, Crime etc. — assign to first sector
          await _models.SectorOfficer.create({ sectorId: Object.values(sectorMap)[0]._id, officerId: officer._id, role: 'PRIMARY_SI' });
        }
      }
    }
