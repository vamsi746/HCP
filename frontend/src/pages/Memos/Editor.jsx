...
const MemoEditorPage = () => {
  ...
  }<div className="bg-gradient-to-r from-[#1a2a4a] to-[#2d3e5f] px-5 py-3.5 flex items-center justify-between border-l-4 border-amber-500 -mx-3 sm:-mx-4 md:-mx-6 -mt-3 sm:-mt-4 md:-mt-6 px-3 sm:px-4 md:px-6"><div className="flex items-center gap-3"><button onClick={handleBack} className="p-1.5 hover:bg-white/10 text-blue-200 hover:text-white transition"><ArrowLeft size={18} /></button><div><div className="flex items-center gap-3"><h1 className="text-sm font-bold text-white uppercase tracking-wider">{data.policeStation || "Unknown"} PS — Cr.No. {data.crimeNo || "\u2014"}</h1><span className={`inline-flex items-center gap-1 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${statusMeta.bg}`}>{statusMeta.icon}{statusMeta.label}</span></div><p className="text-[11px] text-blue-200 mt-0.5">{data.reference}</p></div></div><div className="flex items-center flex-wrap gap-2"><MemoPrintButton
    content={content}
  ...
  ><Send size={13} />{submitMutation.isPending ? "Submitting\u2026" : "Submit for Review"}</button></>}</div></div>{
    ...
    /* Memo editor */
  }<div className="mt-5 mx-0 mb-4 bg-white border border-slate-200 shadow-md"><div className="bg-indigo-600 px-4 py-1.5"><p className="text-[10px] font-bold text-white uppercase tracking-wider">Memorandum Document</p></div><div className="p-2">{content ? <MemoEditor content={content} onUpdate={handleContentUpdate} editable={editable} /> : <div className="border border-gray-200 bg-gray-100 flex items-center justify-center" style={{ minHeight: "min(600px, 70vh)" }}><Loader2 size={24} className="animate-spin text-gray-400" /></div>}</div></div>{
