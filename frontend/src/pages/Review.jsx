...
const Review = () => {
  ...
  const [casePopup, setCasePopup] = useState(null);
  ...
  const onResizeStart = useCallback((dir) => (e) => {
    ...
    const onMove = (ev) => {
      ...
      if (dir.includes("l")) {
        const nw = Math.max(320, w - dx);
        x = x + (w - nw);
        w = nw;
      }
      if (dir.includes("t")) {
        const nh = Math.max(200, h - dy);
        y = y + (h - nh);
        h = nh;
      }
  ...
  const openCasePopup = useCallback((memo) => {
    setCasePopup(memo);
    setPanelMode("split");
  ...
  const minimizePanel = useCallback(() => {
    ...
    const maxY = window.innerHeight - popupSize.height - 100;
  ...
  const assignMutation = useMutation({
    mutationFn: ({ id, recipientType, recipientId }) => assignMemoRecipient(id, { recipientType, recipientId }),
    onSuccess: (res) => {
  ...
  const closeDialog = () => {
    setDialog((prev) => ({ ...prev, open: false, onConfirm: void 0 }));
  };
  ...
  const openComplianceReview = async (memo) => {
    setSelectedId(memo._id);
    setLoadingDetail(true);
  ...
  const handleViewComplianceDoc = async (memo) => {
    try {
  ...
  const stripDocxInlineStyles = (container) => {
    ...
    for (let i = sections.length - 1; i >= 0; i--) {
      if (!(sections[i].textContent || "").trim()) sections[i].style.display = "none";
      else break;
    }
  };
  useEffect(() => {
    if (splitDocxRef.current && complianceDocBlob && isDocxFile(complianceDocName) && !complianceDocLoading) {
      ...
      renderAsync(complianceDocBlob, splitDocxRef.current, void 0, {
  ...
  const renderCaseDetailBody = (memo) => <div className="flex-1 overflow-y-auto scroll-smooth p-4 space-y-4 bg-[#F4F5F7]" style={{ scrollbarWidth: "thin", scrollbarColor: "#c1c7cf transparent" }}><fieldset className="border border-[#D9DEE4] bg-white px-4 pb-3 pt-1 rounded-sm"><legend className="text-[11px] font-bold text-[#003366] uppercase tracking-wider px-1">Nature of Case</legend><p className="text-sm text-[#4A5568]">{memo.caseDetails?.natureOfCase || "\u2014"}</p></fieldset><fieldset className="border border-[#D9DEE4] bg-white px-4 pb-3 pt-1 rounded-sm"><legend className="text-[11px] font-bold text-[#003366] uppercase tracking-wider px-1">Name of the P.S., Cr. No, U/Sec & D.O.R</legend><p className="text-sm text-[#4A5568]">{memo.caseDetails?.psWithCrDetails || `${memo.policeStation || ""} PS Cr.No. ${memo.crimeNo || ""} U/s ${memo.sections || ""} ${memo.caseDetails?.dor ? `DOR.${memo.caseDetails.dor}` : ""}`}</p></fieldset><fieldset className="border border-[#D9DEE4] bg-white px-4 pb-3 pt-1 rounded-sm"><legend className="text-[11px] font-bold text-[#003366] uppercase tracking-wider px-1">Details of Accused</legend><div className="flex items-center gap-2 mb-2">{(memo.caseDetails?.numAccused ?? 0) > 0 && <span className="text-[10px] font-bold bg-[#1B6B46] text-white px-2 py-0.5 rounded-sm">{memo.caseDetails.numAccused} Accused</span>}{(memo.caseDetails?.numCases ?? 0) > 0 && <span className="text-[10px] font-bold bg-[#003366] text-white px-2 py-0.5 rounded-sm">{memo.caseDetails.numCases} Cases</span>}<span className="text-[10px] font-bold bg-[#9B2C2C] text-white px-2 py-0.5 rounded-sm">{memo.caseDetails?.abscondingAccused ?? 0} Absconding</span></div><p className="text-sm text-[#4A5568] whitespace-pre-wrap">{memo.caseDetails?.accusedParticulars || "\u2014"}</p></fieldset><fieldset className="border border-[#D9DEE4] bg-white px-4 pb-3 pt-1 rounded-sm"><legend className="text-[11px] font-bold text-[#003366] uppercase tracking-wider px-1">Brief Facts</legend><p className="text-sm text-[#4A5568] whitespace-pre-wrap">{memo.caseDetails?.briefFacts || memo.briefFacts || "\u2014"}</p></fieldset>{(memo.caseDetails?.seizedProperty || memo.caseDetails?.seizedWorth) && <fieldset className="border border-[#D9DEE4] bg-white px-4 pb-3 pt-1 rounded-sm"><legend className="text-[11px] font-bold text-[#003366] uppercase tracking-wider px-1">Property Seized & Worth</legend>{memo.caseDetails?.seizedWorth && <span className="inline-block text-[10px] font-bold bg-[#A66914] text-white px-2 py-0.5 mb-2 rounded-sm">Rs. {memo.caseDetails.seizedWorth}</span>}<p className="text-sm text-[#4A5568] whitespace-pre-wrap">{memo.caseDetails?.seizedProperty || "\u2014"}</p></fieldset>}{
    /* Case info */
  ...
  }<div ref={docScrollRef} className="flex-1 overflow-auto bg-white" style={{ scrollbarWidth: "thin", scrollbarColor: "#c1c7cf transparent", minWidth: 0 }}>{dsrDocLoading && <div className="flex items-center justify-center h-40 text-slate-400"><Loader2 size={20} className="animate-spin mr-2" />
            Loading case rows…
          </div>}{caseRowsHtml && <div
  ...
  const renderAssignModal = () => {
    ...
    return <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => {
      setAssignModal(null);
      setOfficers(null);
    }}><div className="bg-white shadow-2xl w-[95vw] max-w-md border border-neutral-200 rounded-sm" onClick={(e) => e.stopPropagation()}><div className="bg-[#003366] px-5 py-3 flex items-center justify-between rounded-t-sm"><h2 className="text-sm font-bold text-white uppercase tracking-wider">Issue To</h2><button onClick={() => {
      setAssignModal(null);
      setOfficers(null);
    }} className="text-white/50 hover:text-white"><X size={18} /></button></div><div className="p-5"><p className="text-sm text-neutral-600 mb-4">
              Assign memo to officer at <strong className="text-[#1C2334]">{assignModal.policeStation} PS</strong></p>{loadingOfficers ? <div className="flex justify-center py-8"><Loader2 size={24} className="animate-spin text-neutral-400" /></div> : <div className="space-y-2">{officers?.sho && <button
    ...
    ><div className="flex items-center justify-between"><div><span className="text-[11px] font-bold text-[#003366] uppercase tracking-wider">SHO</span><p className="font-bold text-[#1C2334]">{officers.sho.name}</p><p className="text-xs text-[#718096]">{officers.sho.rank} • {officers.sho.badgeNumber}</p></div><UserCheck size={20} className="text-[#003366]" /></div></button>}{officers?.si && <button
      onClick={() => assignMutation.mutate({ id: assignModal._id, recipientType: "SI", recipientId: officers.si._id })}
      disabled={assignMutation.isPending}
      className="w-full text-left p-4 border border-[#D9DEE4] bg-white hover:bg-[#EBF0F5] hover:border-[#003366]/30 transition rounded-sm"
    ><div className="flex items-center justify-between"><div><span className="text-[11px] font-bold text-[#4A5568] uppercase tracking-wider">SI</span><p className="font-bold text-[#1C2334]">{officers.si.name}</p><p className="text-xs text-[#718096]">{officers.si.rank} • {officers.si.badgeNumber}</p></div><UserCheck size={20} className="text-neutral-400" /></div></button>}{!officers?.sho && !officers?.si && <p className="text-sm text-neutral-500 text-center py-4">No officers found for this police station.</p>}</div>}</div></div></div>;
  };
  const renderDialog = () => {
    if (!dialog.open) return null;
    return <div className="fixed inset-0 bg-black/40 z-[70] flex items-center justify-center" onClick={closeDialog}><div className="w-[95vw] max-w-md bg-white border border-[#D9DEE4] shadow-2xl rounded-sm" onClick={(e) => e.stopPropagation()}><div className="bg-[#003366] px-5 py-3 rounded-t-sm"><h3 className="text-sm font-bold text-white uppercase tracking-wider">{dialog.title}</h3></div><div className="p-5"><p className="text-sm text-[#4A5568] leading-relaxed">{dialog.message}</p><div className="mt-5 flex items-center justify-end gap-2">{dialog.showCancel && <button
      onClick={closeDialog}
    ...
    >{dialog.confirmLabel}</button></div></div></div></div>;
  };
  if (selectedId && memoDetail) {
    ...
    />{(memoDetail.status === "PENDING_REVIEW" || memoDetail.status === "REVIEWED" || memoDetail.status === "ON_HOLD" || memoDetail.status === "REJECTED") && <button
    ...
    ><UserCheck size={13} />
                Issue To
              </button>}{(memoDetail.status === "REVIEWED" || memoDetail.status === "PENDING_REVIEW" || memoDetail.status === "ON_HOLD") && <><button
    ...
    ><CheckCircle2 size={13} />{approveMutation.isPending ? "Approving\u2026" : "Approve"}</button><button
      className="flex items-center gap-1.5 px-4 py-1.5 bg-[#A66914] text-white text-xs font-bold uppercase tracking-wider hover:bg-[#8C5810] disabled:opacity-50 transition rounded-sm"
      onClick={() => confirmHold(memoDetail._id)}
      disabled={holdMutation.isPending}
    ><Clock size={13} />{holdMutation.isPending ? "Holding\u2026" : "Hold"}</button><button
      className="flex items-center gap-1.5 px-4 py-1.5 bg-[#9B2C2C] text-white text-xs font-bold uppercase tracking-wider hover:bg-[#832424] disabled:opacity-50 transition rounded-sm"
      onClick={() => confirmReject(memoDetail._id)}
      disabled={rejectMutation.isPending}
    ><Ban size={13} />{rejectMutation.isPending ? "Rejecting\u2026" : "Reject"}</button></>}</div></div>{
      /* Memo content */
    }<div className="flex-1 overflow-y-auto scroll-smooth min-w-0" style={{ scrollbarWidth: "thin", scrollbarColor: "#c1c7cf transparent" }}><div className={`bg-white min-w-0 ${panelMode === "split" || isComplianceSplit ? "memo-viewer-responsive" : ""}`}><MemoEditor content={memoDetail.content} onUpdate={() => {
    }} editable={false} /></div></div></div>;
    return <>{isComplianceSplit ? (
      ...
      <div ref={splitContainerRef} className="flex -m-3 sm:-m-4 md:-m-6 w-[calc(100%+1.5rem)] sm:w-[calc(100%+2rem)] md:w-[calc(100%+3rem)]" style={{ height: "calc(100vh - 80px - 82px)" }}>{
        /* Left: Memo */
      }<div className="h-full flex flex-col overflow-hidden" style={{ width: `${splitPercent}%` }}>{leftContent}</div>{
        /* Draggable divider */
      }<div
      ...
      }<div className="h-full flex flex-col min-w-0" style={{ width: `${100 - splitPercent}%` }}><div className="bg-[#003366] px-5 py-3 flex items-center justify-between flex-shrink-0 border-b-2 border-[#B8860B]"><h2 className="text-sm font-bold text-white uppercase tracking-wider truncate mr-2">Compliance Document</h2><div className="flex items-center gap-2">{complianceDocUrl && <a
      ...
      ><Download size={11} /> Download
                    </a>}</div></div><div className="flex-1 min-h-0 bg-white">{complianceDocLoading ? <div className="flex items-center justify-center h-full text-slate-400"><Loader2 size={24} className="animate-spin mr-2" /><span className="text-sm">Loading document…</span></div> : complianceDocUrl ? complianceDocName.toLowerCase().endsWith(".pdf") ? <iframe src={`${complianceDocUrl}#navpanes=0`} className="w-full h-full border-0" title="Compliance Document" /> : isDocxFile(complianceDocName) ? <div ref={splitDocxRef} className="w-full h-full overflow-auto bg-white" style={{ scrollbarWidth: "thin", scrollbarColor: "#c1c7cf transparent" }} /> : <div className="flex flex-col items-center justify-center h-full text-slate-500 gap-3"><FileText size={48} className="text-slate-300" /><p className="text-sm font-semibold">{complianceDocName}</p><p className="text-[12px] text-slate-400">Preview not available for this file type</p><a
        href={complianceDocUrl}
        download={complianceDocName}
      ...
      >
                        Download File
                      </a></div> : <div className="flex flex-col items-center justify-center h-full text-slate-400 gap-2"><FileText size={40} className="text-slate-300" /><p className="text-sm font-semibold">No compliance document attached</p>{memoDetail.complianceRemarks && <div className="mt-4 max-w-md bg-slate-50 border border-slate-200 p-4 rounded-sm"><p className="text-[11px] font-bold text-[#003366] uppercase tracking-wider mb-1.5">Compliance Remarks</p><p className="text-[13px] text-slate-600 whitespace-pre-wrap">{memoDetail.complianceRemarks}</p></div>}</div>}</div></div></div>
    ) : isSplit ? <div ref={splitContainerRef} className="flex -m-3 sm:-m-4 md:-m-6 w-[calc(100%+1.5rem)] sm:w-[calc(100%+2rem)] md:w-[calc(100%+3rem)]" style={{ height: "calc(100vh - 80px - 82px)" }}>{
      /* Left panel */
    }<div className="h-full flex flex-col overflow-hidden" style={{ width: `${splitPercent}%` }}>{leftContent}</div>{
      /* Draggable divider */
    }<div
  ...
  const listContent = <div className="flex flex-col h-full">{
    /* Page header — sticky */
  }<div className="bg-[#003366] px-5 py-3 border-b-2 border-[#B8860B] flex-shrink-0 sticky top-0 z-10"><h1 className="text-sm font-bold text-white uppercase tracking-wider">To Be Reviewed by CP Sir</h1></div>{
    /* Scrollable content */
  }<div className="flex-1 overflow-y-auto scroll-smooth p-5" style={{ scrollbarWidth: "thin", scrollbarColor: "#c1c7cf transparent" }}>{
    /* Tabs */
  }<div className="flex flex-wrap items-center gap-0 mb-5 border-b border-neutral-200">{TABS.map((t) => <button
    key={t.key}
    onClick={() => setTab(t.key)}
    className={`px-5 py-2.5 text-xs font-bold uppercase tracking-wider border-b-[3px] transition-all ${tab === t.key ? "border-[#003366] text-[#003366] bg-[#003366]/5" : "border-transparent text-[#718096] hover:text-[#4A5568] hover:border-[#D9DEE4]"}`}
  >{t.label}<span className={`ml-2 text-white text-[10px] px-1.5 py-0.5 font-bold rounded-sm ${tab === t.key ? "bg-[#003366]" : "bg-[#718096]"}`}>{getTabCount(t.key)}</span></button>)}</div>{
    /* Filter bar */
  ...
  ><RotateCcw size={12} />
    ...
    /* Cards grid */
  }{isLoading ? <div className="bg-white border border-neutral-200 rounded-sm p-16 text-center text-neutral-400"><Loader2 size={28} className="animate-spin mx-auto mb-2" /><p className="text-xs uppercase tracking-wider font-semibold">Loading memos…</p></div> : memos.length === 0 ? <div className="bg-white border border-neutral-200 rounded-sm p-16 text-center"><FileText size={40} className="mx-auto text-neutral-300 mb-3" /><p className="text-sm text-neutral-500 font-semibold">No memos in this category.</p></div> : <div className={`grid gap-3 p-1 ${isSplitList ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5"}`}>{memos.map((memo) => {
    ...
    ><BookOpen size={14} /></button><button
  ...
  return <>{isSplitList ? <div ref={splitContainerRef} className="flex -m-3 sm:-m-4 md:-m-6 w-[calc(100%+1.5rem)] sm:w-[calc(100%+2rem)] md:w-[calc(100%+3rem)]" style={{ height: "calc(100vh - 80px - 82px)" }}>{
    /* Left panel */
  }<div className="h-full flex flex-col overflow-hidden" style={{ width: `${splitPercent}%` }}>{listContent}</div>{
    /* Draggable divider */
  }<div
    className="h-full w-[5px] bg-[#D9DEE4] hover:bg-[#003366] active:bg-[#003366] cursor-col-resize flex-shrink-0 relative group transition-colors duration-150"
    onMouseDown={onSplitDragStart}
  ...
  }{previewDoc && <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[80]" onClick={() => {
    window.URL.revokeObjectURL(previewDoc.url);
    setPreviewDoc(null);
  }}><div className="bg-white shadow-2xl flex flex-col w-[95vw] max-w-7xl h-[92vh]" onClick={(e) => e.stopPropagation()}><div className="bg-[#003366] px-5 py-3 flex items-center justify-between flex-shrink-0"><h3 className="text-sm font-bold text-white uppercase tracking-wider truncate">{previewDoc.name}</h3><div className="flex items-center gap-2"><a
    href={previewDoc.url}
  ...
  ><Download size={11} /> Download
    ...
    window.URL.revokeObjectURL(previewDoc.url);
    setPreviewDoc(null);
  }} className="text-white/50 hover:text-white transition"><X size={18} /></button></div></div><div className="flex-1 min-h-0">{previewDoc.name.toLowerCase().endsWith(".pdf") || previewDoc.url.includes("application/pdf") ? <iframe src={previewDoc.url} className="w-full h-full border-0" title="Document Preview" /> : isDocxFile(previewDoc.name) ? <div ref={modalDocxRef} className="w-full h-full overflow-auto bg-white" style={{ scrollbarWidth: "thin", scrollbarColor: "#c1c7cf transparent" }} /> : <div className="flex flex-col items-center justify-center h-full text-slate-500 gap-3"><FileText size={48} className="text-slate-300" /><p className="text-sm font-semibold">Preview not available for this file type</p><a
    href={previewDoc.url}
    download={previewDoc.name}
    className="px-4 py-2 bg-[#003366] text-white text-[12px] font-bold uppercase tracking-wider hover:bg-[#004480] transition rounded-sm"
  >
